...
//	sbuffer = (char*)malloc(bufsize);
	...
	snodeValues = (ob*)malloc(4 * snodes);//for deserializing
	toSerialize = snodeValues;//for serializing
	serializedBindings = mk_Sarray(snodes);
...
// if s is nul, this means that use of the C toSerialize array is intended instead
